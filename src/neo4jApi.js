...
function inserer(film,producteur,realisateur){

  console.log("titre"+producteur.name)
  const session = driver.session({database: database});
  return session.readTransaction((tx) =>
  tx.run('CREATE (:Person { name: $namep , born: $pborn})-[:PRODUCED]->(:Movie {title:$titre, released: $released, tagline :$tagline } )'+
  '<-[:WROTE]-(:Person {name:$namer, born :$rborn})', {
    titre: film.titre, released: film.released, tagline: film.tagline, namep : producteur.name,
    pborn : producteur.born,
    rborn : realisateur.born,
    namer : realisateur.name
  })
    .then(result => {
